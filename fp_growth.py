...
def find_frequent_itemsets(transactions, minimum_support, include_support=False, include_card = False, no_dopples = False):
	"""
	...
	If `include_card` is true, yield (itemset, cardinality of itemset) pairs instead of
	just the itemsets.
	...
	E.G.
		...
		becomes
		[a,b,c] 2
		[a,b]   3   (original 5 minus 2 dopples from [a,b,c])
		[a,c]   4	(original 6 minus 2 dopples from [a,b,c])
		[a]     6   (original 15 minus 2 from [a,b,c] minus 3 from updated [a,b] minus 4 from updated [a,c])
	"""
	...
	def find_with_suffix(tree, suffix):
		for item, nodes in list(tree.items()):
			...
			if support >= minimum_support and item not in suffix:
				...
				if (include_support and include_card) or no_dopples:
					yield (found_set, support, len(found_set))
				if include_support and not include_card and not no_dopples:
					yield (found_set, support)
				if not include_support and include_card and not no_dopples:
					yield (found_set, len(found_set))
				if not include_support and not include_card and not no_dopples:
					yield (found_set)
	...
	# no_dopples removes support from lower cardinality itemset equal to the support of higher cardinality set which are supersets of them
	# in this way the support for an itemset is the actual number of times that item has appeared alone
	if no_dopples:
		no_dopples_list = list()
		for itemset in find_with_suffix(master, []):
			no_dopples_list.append(itemset)
		no_dopples_list.sort(key=itemgetter(2), reverse=True)
		for t in range(len(no_dopples_list)):
			for s in range(0, t):
				if set(no_dopples_list[t][0]).issubset(set(no_dopples_list[s][0])) and set(no_dopples_list[t][0]) != set(
						no_dopples_list[s][0]):
...
if __name__ == '__main__':
	...
	p.add_option('-d', '--no-dopples', dest = 'no_dopples', action = 'store_true',
		help='Support of the patterns equals the actual number of its appearances alone disregarding appearance in supersets (default: false)')
	p.set_defaults(minsup=2)
	p.set_defaults(numeric=False)
	p.set_defaults(no_dopples=False)
	...
	result = []
	for itemset, support in find_frequent_itemsets(transactions, options.minsup, True, False, options.no_dopples):
